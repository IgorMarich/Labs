...
int main()
{
    double operands[] = { 10.5, 2.5, -3, 1.5 };
    double operands1[] = { 5,15,-8, 1, 2, 3 };

    ExpressionEvaluator* expression[3];
    expression[0] = new CustomExpressionEvaluator();
    expression[0]->setOperands(operands1, 6);
   

    expression[1] = new Summator();
    expression[1]->setOperand(0, 1.5);
    expression[1]->setOperand(1, -4);
    expression[1]->setOperand(2, 2.5);


    expression[2] = new Subtractor();
    expression[2]->setOperands(operands, 4);

    string  a[3] = { "CustomExpressionEvaluator:", " Summator:","Subtractor:"};
    for (int i = 0; i < 3; i++)
    {
        cout << a[i] << endl;
        expression[i]->logToScreen();
        expression[i]->logToFile("lab3");
    }
    for (int i = 0; i < 3; i++) {
